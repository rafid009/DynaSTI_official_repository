...
class DynaSTI(nn.Module):
    ...
    def forward(self, inputs, t, is_train=1):
        ...
        print(f"spatial input: {spatial_input.device}, spatial_context_embeddimg device: {self.spatial_context_embeddimg.weight.device}, {self.spatial_context_embeddimg.bias.device}")
...
class Guide_diff(nn.Module):
    def __init__(self, config, inputdim=1, target_dim=36, is_itp=False):
        ...
        if self.is_itp:
            self.itp_channels = config["diffusion"]["channels"]
            self.itp_projection = Conv1d_with_init(inputdim-1, self.itp_channels, 1)

            self.itp_modeling = GuidanceConstruct(channels=self.itp_channels, nheads=config["diffusion"]["nheads"], target_dim=target_dim,
                                            order=2, include_self=True, device=device, is_adp=config["diffusion"]["is_adp"],
                                            adj_file=config["adj_file"], proj_t=config["diffusion"]["proj_t"])
        ...
        if config["adj_file"] == 'AQI36':
            ...
            self.adj = get_adj_metrla()
        elif config["adj_file"] == 'pems-bay':
            self.adj = get_adj_pemsbay()
    ...
    def forward(self, x, side_info, diffusion_step, itp_x, cond_mask=None):
        if self.is_itp:
