            ...
            for j, test_batch in enumerate(it, start=1):
                ...
                if 'DynaSTI-Orig' in models.keys():
                    with torch.no_grad():
                        ...
                        output_diff_saits_orig = models['DynaSTI-Orig'].evaluate(test_batch, nsample, latent_size=latent_size, miissing_dim=missing_dims)
                        spat_end = time.time()
                ...
                if 'SPAT-SADI' in models.keys():
                    with torch.no_grad():
                        spat_start = time.time()
                        output_diff_saits = models['SPAT-SADI'].evaluate(test_batch, nsample, latent_size=latent_size, miissing_dim=missing_dims)
                        spat_end = time.time()
